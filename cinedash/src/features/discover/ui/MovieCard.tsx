...
export function MovieCard({ movie }: MovieCardProps) {
  ...
  const isInWatchlist = useWatchlistStore((state) => state.movies.some((m) => m.id === movie.id))
  ...
  return (
    <Card className="group flex flex-col overflow-hidden transition-shadow hover:shadow-md">
      <Link to="/movie/$id" params={{ id: movie.id }} className="relative block overflow-hidden">
        {poster ? (
          <img
            src={poster}
            alt={movie.title}
      ...
      <CardContent className="flex flex-1 flex-col gap-1 p-3">
        <Link to="/movie/$id" params={{ id: movie.id }} className="line-clamp-1 text-sm font-medium hover:underline">
          {movie.title}
        </Link>
